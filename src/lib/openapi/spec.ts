...
export const openApiSpec = {
    ...
    schemas: {
      ...
      CreateUser: {
        ...
        properties: {
          ...
          phone: { type: 'string' },
          jobTitle: { type: 'string', description: 'e.g. "Office Manager"' },
          notes: { type: 'string', description: 'Internal admin note (not self-editable)' },
        },
      },
      UpdateUser: {
        type: 'object',
        properties: {
          ...
          role: {
            type: 'string',
            enum: ['superAdmin', 'admin', 'accountant', 'sales', 'readOnly'],
          },
          status: { type: 'string', enum: ['invited', 'active', 'disabled'] },
          avatarUrl: {
            type: 'string',
            nullable: true,
            description: 'base64 image data URL (≤~500KB), or null to clear',
          },
        },
      },
      InvoiceItem: {
      ...
      RevokeSessions: {
        ...
        properties: {
          scope: {
            type: 'string',
            enum: ['others', 'all'],
            description: 'others = keep this device; all = sign out everywhere',
          },
        },
      },
      RequestEmailChange: {
  ...
  paths: {
    ...
    '/api/auth/verify': {
      post: {
        tags: ['Auth'],
        summary: 'Onboarding: verify OTP + set password',
    ...
    '/api/auth/me': {
      get: {
        tags: ['Auth'],
        summary: 'Current session + profile + permissions',
    ...
    '/api/auth/email/request': {
      post: {
        tags: ['Auth'],
        summary:
          'Start an email change — verifies password, emails a code to the new address (requires session)',
        requestBody: {
          content: {
            'application/json': { schema: { $ref: '#/components/schemas/RequestEmailChange' } },
          },
        },
        responses: {
          200: { description: 'Code sent' },
          400: { description: 'Wrong password / email taken' },
          401: { description: 'Unauthorized' },
        },
      },
    },
    '/api/auth/email/confirm': {
      post: {
        ...
        requestBody: {
          content: {
            'application/json': { schema: { $ref: '#/components/schemas/ConfirmEmailChange' } },
          },
        },
        responses: {
    ...
    '/api/auth/sessions': {
      get: {
        tags: ['Auth'],
        summary: 'Your active sessions + recent revokes (requires session)',
        responses: { 200: { description: 'OK' }, 401: { description: 'Unauthorized' } },
      },
    },
    '/api/auth/sessions/revoke': {
      post: {
        ...
        requestBody: {
          content: {
            'application/json': { schema: { $ref: '#/components/schemas/RevokeSessions' } },
          },
        },
        responses: { 200: { description: 'OK' }, 401: { description: 'Unauthorized' } },
      },
                ...
                schema: {
                  ...
                  properties: {
                    ...
                    pipelineMonth: {
                      type: 'string',
                      format: 'date-time',
                      description: 'ISO start of the month byState covers',
                    },
                    byState: {
                      type: 'object',
    ...
    '/api/auth/users/{id}': {
      ...
      delete: {
        tags: ['Users'],
        summary: 'Deactivate user (soft delete, UserManage)',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
        responses: { 200: { description: 'OK' } },
      },
    },
    '/api/invoices': {
      get: {
        ...
        parameters: [
          ...
          { name: 'type', in: 'query', schema: { type: 'string', enum: ['tax', 'cash', 'pk'] } },
          {
            ...
            description:
              'archived = Admin+ or creator; deleted = admins only; all = everything the caller may see',
          },
          { name: 'search', in: 'query', schema: { type: 'string' } },
          {
            name: 'from',
            ...
            description: 'Filters createdAt from the start of this UTC day (inclusive)',
          },
          {
            name: 'to',
            in: 'query',
            schema: { type: 'string', format: 'date' },
            description: 'Filters createdAt to the end of this UTC day (inclusive)',
          },
        ],
        responses: {
      ...
      post: {
        ...
        requestBody: {
          content: {
            'application/json': { schema: { $ref: '#/components/schemas/CreateInvoice' } },
          },
        },
    ...
    '/api/invoices/export': {
      get: {
        tags: ['Invoices'],
        summary:
          'Export invoices as a file (InvoiceView) — same filters as the list, no pagination',
        description:
          'Streams the matching invoices as a download. Capped at EXPORT_LIMIT (5000) rows; check the X-Export-Truncated header to detect a clipped file.',
        parameters: [
          {
            name: 'format',
            in: 'query',
        ...
        responses: {
          200: {
            ...
            headers: {
              ...
              'X-Export-Truncated': {
                schema: { type: 'boolean' },
                description: 'True when the cap clipped the file',
              },
            },
            content: {
    ...
    '/api/invoices/{id}': {
      get: {
        tags: ['Invoices'],
        summary: 'Get invoice (InvoiceView)',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
        responses: { 200: { description: 'OK' } },
      },
      patch: {
        tags: ['Invoices'],
        summary: 'Update invoice (InvoiceEdit)',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
        responses: { 200: { description: 'OK' } },
      },
      delete: {
        ...
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
        requestBody: {
          content: { 'application/json': { schema: { $ref: '#/components/schemas/Reason' } } },
        },
