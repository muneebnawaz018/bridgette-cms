...
// Finalizing an invoice now renders a PDF and sends it inline, which needs a real Node process
...
export const POST = handle(async (req) => {
   ...
   * as an outcome and the invoice still comes back 201. "Email to customer" on the list is the
   * retry.
   */
  let emailed: EmailOutcome = { sent: false };
  if (body.asDraft) {
    emailed.reason = 'Drafts are not sent to customers.';
  } else if (!invoice.billTo?.email) {
