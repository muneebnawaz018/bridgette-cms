...
31. **A customer may hold up to 10 delivery addresses.** "Ships to the billing address" means none are stored, and every reader falls back to the billing party rather than to a copy that goes stale. Raising an invoice fills SHIP TO with the customer's first address (or the billing party when they have none) and offers the rest in a dropdown; the invoice keeps its own copy of whichever was picked, so correcting a customer's address later never rewrites where something already shipped.
32. **An invoice carries at most one shipment.** Tracking ID, agent and shipping date are required; the ETA is optional, because an agent often has not given one at booking, and it can never fall before the shipping date. Shipping is recorded and corrected after the invoice is finalized, so it lives in its own record rather than on the invoice. Nothing ships against a draft: a draft carries no commitment to the customer and its lines, totals and even its customer can still change, so goods leaving against one would be a consignment against a document that does not exist yet. An archived or deleted invoice refuses shipping changes for the opposite reason, the same one it refuses payments for: it is history. All three are enforced on write, and the menu entry is not offered for a draft. Reading shipping follows the invoice's own visibility rule exactly, so a tracking number is never reachable by anyone who could not open the invoice.
